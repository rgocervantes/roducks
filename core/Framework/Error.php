...
">';
	...
	{
		$markup = '<!DOCTYPE html><html><head><title>Roducks Debugger</title></head><body style="background: #1f2126; color: #fff;">';
		$markup .= '<div style="text-align:center; margin-top: 40px;">';
			...
			$markup .= '<div style="display:inline-block; min-width: 760px;">';
	...
	{
		if (Environment::inCLI()) {
			CLI::printError($title, CLI::FAILURE, $items);
		} else {
			echo self::_throw("{$title}:", $line, $path, $file, $error);
			exit;
		}
	}
	...
	static function defaultPageIsMissing($title, $line, $path, $file)
	{
		...
		$error = "<span style=\"color:#3bde9a; \">use</span> Roducks\Framework\Router;<br>";
		$error .= "<span style=\"color:#3bde9a; \">use</span> Roducks\Framework\Dispatch;<br><br>";
